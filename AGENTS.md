...
4. **Persistence**: `:w` calls `save_session()`, writes JSON to either `~/.local/share/tuicr/reviews/` (default) or `.tuicr/reviews/` in the repository when `local_storage` is enabled
